...
- "附近"按钮获取浏览器定位，结果带直线距离与**步行耗时估算**（按 80 米/分钟 + 1.3 绕行系数），按步行耗时排序，默认半径 1000 米，可调 200/500/1000/3000 米。
- 出入口有台阶或坡道的条目在结果列表与详情页标注**通行提示**（台阶为注意、坡道为可通行），数据来自分类属性里的 `entrance_steps` / `entrance_ramp`。
...
五个分类都另有共用的出入口通行属性：`entrance_steps`（出入口有台阶）与 `entrance_ramp`（出入口有坡道），用于生成通行提示。
...
带 `near` 时每条结果额外返回 `distanceMeters`（直线距离）与 `walkingMinutes`（步行耗时估算），`sort=distance` 即按步行耗时升序；所有条目均返回 `accessNotes`（出入口台阶/坡道的通行提示，无相关属性时为空数组）。
