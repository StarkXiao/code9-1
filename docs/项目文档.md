...
五个分类还共享两个可选的出入口通行属性：**到达处有台阶**（`entrance_steps`）、**出入口有坡道**（`entrance_ramp`）。附近搜索与详情页据此展示台阶/坡道通行提示。
...
| `sort` | `freshness`(默认) / `distance` / `newest`；传入 `near` 时一律按步行耗时从短到长排序 |
...
| 字段 | 说明 |
|---|---|
| `distanceMeters` | 到定位点的直线距离（米，四舍五入） |
| `walkingMinutes` | 步行可达估算耗时（分钟）：直线距离 × 1.3 绕行系数 ÷ 75 米/分钟，至少 1 分钟，仅用于排序与参考，不是路线规划 |

条目属性中标注了出入口通行情况时（`entrance_steps` 有台阶 / `entrance_ramp` 有坡道，各分类 Schema 均可选），列表与详情还会返回 `accessNotice` 文案，例如"入口有台阶，通行请注意"、"入口有坡道，轮椅、婴儿车可通行"。
